...
regions.forEach(([code, name]) => {
  const region = regionMap[code] = { code, name }
  ...
  if (code.slice(4, 6) === '00') {
    const province = regionMap[code.slice(0, 2) + '0000']
    if (!province) return
  ...
  let city = regionMap[code.slice(0, 4) + '00'] // || regionMap[code.slice(0, 2) + '0000']

  if (!city) {
    const cityCode = code.slice(0, 4) + '00'
    const province = regionMap[code.slice(0, 2) + '0000']
    if (!province.children) province.children = []
    city = regionMap[cityCode] = { code: cityCode, name: province.name }
...
// require('fs').writeFileSync(__dirname + '/regions.json', JSON.stringify(regionList), 'utf8')
